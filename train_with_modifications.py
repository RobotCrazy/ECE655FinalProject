#In this file, we will be testing a modified model. This model will contain half the number of parameters in hidden_size.
#This model will freeze the CNN portion of the model and only train the modified portion of the RNN
...
valid_dl = DataLoader(valid_dataset, batch_sampler=valid_sampler)
...
if par.pretrained_flownet and not par.resume:
	if use_cuda:
		pretrained_w = torch.load(par.pretrained_flownet, weights_only=False)
	else:
...
#Modify LSTM to have 500 hidden nodes
M_deepvo.rnn = nn.LSTM(
                    input_size=int(np.prod(30720)), 
                    hidden_size=par.rnn_hidden_size_modified1, 
                    num_layers=2, 
